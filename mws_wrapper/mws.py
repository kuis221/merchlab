...
class Products(MWS):
    ...
    def get_my_fees_estimate(self, feesEstimateRequestList):
        data = dict(Action='GetMyFeesEstimate')
        #print(feesEstimateRequestList)
        data.update(self.enumerate_tree2dict('FeesEstimateRequestList.FeesEstimateRequest.', feesEstimateRequestList))
        #print(data)
        return self.make_request(data)
...
class InboundShipments(MWS):
    ...
    def get_unique_package_labels(self, shipmentid, page_type, package_labels_to_print):
        """ Returns information on available inventory """
        ...
        data.update(
            self.enumerate_param('PackageLabelsToPrint.member.', package_labels_to_print))
        data = self.tree2dict('', data)
        print data
    ...
    #def put_transport_content(self, shipmentid, is_partnered, shipment_type, transport_details)
...
class Subscriptions(MWS):
    ...
    def create_subscription(self, marketplaceid, notification_type, sqs_queue_url, is_enabled=True):
        if is_enabled:
            is_enabled = "true"
        else:
            is_enabled = "false"
        data = {
            ...
            "Subscription.Destination.IsEnabled": is_enabled,
            "MarketplaceId": marketplaceid,
        }
        return self.make_request(data, "POST")
    ...
    def send_test_notification_to_destination(self, marketplaceid, sqs_queue_url, is_enabled=True):
        if is_enabled:
            is_enabled = "true"
        else:
            is_enabled = "false"
        data = {
            ...
            "Destination.AttributeList.member.1.Key": "sqsQueueUrl",
            "Destination.AttributeList.member.1.Value": sqs_queue_url,
            "Destination.IsEnabled": is_enabled,
            "MarketplaceId": marketplaceid            
        }
        return self.make_request(data, "POST")
